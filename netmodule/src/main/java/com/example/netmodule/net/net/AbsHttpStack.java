...
public abstract class AbsHttpStack<T> implements INetStack<T> {
    ...
    public void post(final String url, final String json,
            final Net.Parser<T> parser, final Net.Callback<T> callback,
            final Object tag) {
        throw new UnsupportedOperationException(
                "you must implement this operation by yourself!");
    }
    ...
    public void onNetResponse(final Net.Parser<T> parser,
            final Net.Callback<T> callback, final String response) {
        if (isDebugMode())
