...
public class AroundCircleView extends ImageView {
    ...
    public AroundCircleView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mBorderWidth = a.getDimensionPixelSize(
                R.styleable.AroundCircleView_edgeSize, DEFAULT_BORDER_WIDTH);
        mBorderColor = a.getColor(R.styleable.AroundCircleView_edgeColor,
                DEFAULT_BORDER_COLOR);

        mBorderColor_bg = a.getColor(R.styleable.AroundCircleView_edgeBgColor,
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mBorderWidth != 0) {
            //周边底部颜色 一般为白色
            canvas.drawArc(mBorderRect, 0, 360, false, mBorderPaint_bg);
            //设置了周边弧度的宽度 每次重新绘制都要画上边上的弧度
            canvas.drawArc(mBorderRect, 0, newAngle, false, mBorderPaint);
        }
